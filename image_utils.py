...
def random_rotate(img, degrees, rng=np.random.RandomState(100)):
    # randomly rotate from 0 to degrees and from 0 to negative degrees
    deg = rng.uniform(-1.0*degrees, degrees)
    if deg < 0:
      deg = 360 + deg
    return rotate(img, deg, reshape=False, cval=255.0)

def random_translate(image, units, rng1=np.random.RandomState(100), rng2=np.random.RandomState(101)):

    image = cv2.cvtColor(image,cv2.COLOR_GRAY2RGB)
    
    units_x, units_y = units

    x_units = rng1.randint(-units_x, units_x)
    y_units = rng2.randint(-units_y, units_y)
