...
def apply_gradient(update_gradient_vars, grads, optimizer, learning_rate, learning_rate_multipliers=None):
    ...
    for scope, scope_content in learning_rate_dict.items():
        ...
        if optimizer_name=='ADAGRAD':
            ...
            opt = tf.train.AdamOptimizer(scope_learning_rate, **optimizer_params)
        elif optimizer_name=='MAO':
            opt = MovingAverageOptimizer(tf.train.AdamOptimizer(scope_learning_rate, **optimizer_params), average_decay=0.9999)
        elif optimizer_name=='RMSPROP':
            opt = tf.train.RMSPropOptimizer(scope_learning_rate, **optimizer_params)
        elif optimizer_name=='MOM':
            opt = tf.train.MomentumOptimizer(scope_learning_rate, **optimizer_params)
...
def restore_model(sess, var_list, model_dir, restore_scopes=None, exclude_scopes=None, replace=None):
        ...
        if exclude_scopes is not None:
            ...
            for var in var_list:
                ...
                for exclude_scope in exclude_scopes:
                    # print(var)
                    if exclude_scope in var.name:
                        include = False
                if include:
                    l.append(var)
            var_list = l
            # var_list = [var for var in var_list if var not in exclude_scopes]
        # print(var_list)
        # sys.exit(1)
...
def load_model(sess, model_path, scope=None, random_ckpt=False, rng=np.random.RandomState(random.randint(101, 200))):
    ...
    with sess.graph.as_default():
        ...
        if (os.path.isfile(model_path)):
            ...
            # Load graph and variables separately.
